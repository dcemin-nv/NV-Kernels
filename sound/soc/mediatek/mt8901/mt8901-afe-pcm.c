...
static int mt8901_afe_pcm_dev_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		/*
		 * Transient refusal, nothing latched: the device stays
		 * runtime-active with the regcache in write-through mode
		 * until the PM core retries.
		 */
		dev_warn(dev, "initial runtime suspend deferred: %d\n", ret);
		runtime_active = true;
	} else {
		regcache_cache_only(afe->regmap, true);
		regcache_mark_dirty(afe->regmap);
...
err_pm_put:
	/*
	 * Bypass mode is still set here, so a successful put releases the
	 * MTCMOS and register clock through runtime_suspend. A refused put
	 * (-EAGAIN, -EBUSY, or a callback failure that unwound itself) leaves
	 * the device active with both still held: release them by hand.
	 */
